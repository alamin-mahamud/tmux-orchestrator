...
```bash
# List all directories in ~/work/amir-parvin-group/dark-knight to find projects
ls -la ~/work/amir-parvin-group/dark-knight | grep "^d" | awk '{print $NF}' | grep -v "^\."

# If project name is ambiguous, list matches
ls -la ~/work/amir-parvin-group/dark-knight | grep -i "task"  # for "task templates"
```
...
```bash
# 1. Find project
ls -la $PROJECT_PATH | grep -i task
# Found: task-templates

# 2. Create session
tmux new-session -d -s task-templates -c "$HOME/work/task-templates"

# 3. Set up windows
tmux rename-window -t task-templates:0 "Claude-Agent"
tmux new-window -t task-templates -n "Shell" -c "$HOME/work/task-templates"
tmux new-window -t task-templates -n "Dev-Server" -c "$HOME/work/task-templates"
...
# Examples:
# Send to a window
$HOME/work/Tmux\ orchestrator/send-claude-message.sh agentic-seek:3 "Hello Claude!"

# Send to a specific pane in split-screen
$HOME/work/Tmux\ orchestrator/send-claude-message.sh tmux-orc:0.1 "Message to pane 1"

# Send complex instructions
$HOME/work/Tmux\ orchestrator/send-claude-message.sh glacier-backend:0 "Please check the database schema for the campaigns table and verify all columns are present"

# Send status update requests
$HOME/work/Tmux\ orchestrator/send-claude-message.sh ai-chat:2 "STATUS UPDATE: What's your current progress on the authentication implementation?"
```
...
- **Location**: `$HOME/work/Tmux orchestrator/send-claude-message.sh`
...
$HOME/work/Tmux\ orchestrator/send-claude-message.sh project:0 "You are responsible for the frontend codebase. Please start by analyzing the current project structure and identifying any immediate issues."
```
...
$HOME/work/Tmux\ orchestrator/send-claude-message.sh frontend:0 "Which API endpoints are you currently using from the backend?"

# Share info with backend agent
$HOME/work/Tmux\ orchestrator/send-claude-message.sh backend:0 "Frontend is using /api/v1/campaigns and /api/v1/flows endpoints"
```
...
$HOME/work/Tmux\ orchestrator/send-claude-message.sh session:0 "Quick status update please"

# Detailed status request
$HOME/work/Tmux\ orchestrator/send-claude-message.sh session:0 "STATUS UPDATE: Please provide: 1) Completed tasks, 2) Current work, 3) Any blockers"
```
...
$HOME/work/Tmux\ orchestrator/send-claude-message.sh session:0 "I see in your server window that port 3000 is already in use. Try port 3001 instead."

# Guide stuck agents
$HOME/work/Tmux\ orchestrator/send-claude-message.sh session:0 "The error you're seeing is because the virtual environment isn't activated. Run 'source venv/bin/activate' first."
```
...
```bash
# Send message
$HOME/work/Tmux\ orchestrator/send-claude-message.sh session:0 "What's your status?"
